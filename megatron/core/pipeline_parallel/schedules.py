...
def get_forward_backward_func():
    ...
    if pipeline_model_parallel_size > 1:
        if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
            print(f"TdH: using interleaving")
            forward_backward_func = forward_backward_pipelining_with_interleaving
        else:
            print(f"TdH: using no interleaving")
            forward_backward_func = forward_backward_pipelining_without_interleaving
    else:
        print(f"TdH: using no pipelining at all")
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    stages.
    Returns dictionary with losses if the last stage, empty dict otherwise."""
    print(f"TdH: inside forward_backward_pipelining_without_interleaving")
    if isinstance(model, list):
        assert len(model) == 1, (
    ...
    print(f"TdH -- without_interleaving: getting model config")
    config = get_model_config(model)
    print(f"TdH -- got model config", config)
    if config.overlap_p2p_comm:
        raise ValueError(
    ...
    print(f"TdH, rank {rank}: recv_tensor_shapes: {recv_tensor_shapes}")
    ...
    print(f"TdH, rank {rank}: send_tensor_shapes: {send_tensor_shapes}")
    ...
    for i in range(num_warmup_microbatches):
        print(f"TdH, rank {rank}: running warmup forward pass {i}")
        # Decide to checkpoint all layers' activations of the current micro-batch
        if max_outstanding_backprops is not None:
            checkpoint_activations_microbatch = (
        ...
        print(f"TdH, rank {rank}: calling recv_forward")
        input_tensor = recv_forward(recv_tensor_shapes, config)
        print(f"TdH, rank {rank}: calling forward_step")
        output_tensor = forward_step(
            forward_step_func,
            data_iterator,
            model,
        ...
        print(f"TdHTdH, rank {rank}: calling send_forward")
    ...
    for i in range(num_microbatches_remaining):
        # print(f"TdH, rank {rank}: running 1F1B in steady state {i}")
