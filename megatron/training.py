...
def setup_teacher_model(args, model_provider):
    print_rank_0(
        "***>>>>> Student model checkpoint iteration:{}".format(args.iteration)
    )
...
def setup_model_and_optimizer(
    ...
    if args.kd or args.mos:
        ...
        print_rank_0(
            "***>>>>> Student model, global step:{}".format(student_global_steps)
        )
    ...
    if not no_post_init_checkpoint_loading:
        if args.load is not None:
            print_rank_0(f"TdH: loading checkpoint")
            print_rank_0(f"TdH: inspecting model that is expected, including its keys and the shapes of the tensors")
            print_rank_0(f"TdH: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
            print_rank_0(f"TdH: rank tp{mpu.get_tensor_model_parallel_rank()}, pp{mpu.get_pipeline_model_parallel_rank()}")
            state_dict = model[0].state_dict()
            for key, value in state_dict.items():
                assert 0==0 # do something so we can comment out the following print statement automatically without breaking the for loop
                print_rank_0(f"TdH: {key}: {value.shape}")
            print_rank_0(f"TdH: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
            timers = get_timers()
...
def train_step(
    ...
    print(f"TdH: starting forward pass")
    # Forward pass.
    print(f"TdH: starting timer")
    timers("forward-backward", log_level=1).start(barrier=args.barrier_with_L1_time)
    print(f"TdH: getting the right forward-backward function")
    forward_backward_func = get_forward_backward_func()
    print(f"TdH: got the right forward-backward function")
    ...
    print(f"TdH: calling forward_backward_func with ")
    print(f"TdH: forward_step_func: {forward_step_func}")
    print(f"TdH: data_iterator: {data_iterator}")
    print(f"TdH: model: {model}")
    print(f"TdH: num_microbatches: {get_num_microbatches()}")
    print(f"TdH: seq_length: {args.seq_length}")
    print(f"TdH: micro_batch_size: {args.micro_batch_size}")
    print(f"TdH: decoder_seq_length: {args.decoder_seq_length}")
    print(f"TdH: forward_only: {False}")
    ...
    print(f"TdH: calling reduce_model_grads")
    # Reduce gradients.
    if not args.deepspeed:
    ...
    print(f"TdH: updating parameters")
    # Update parameters.
    timers("optimizer", log_level=1).start(barrier=args.barrier_with_L1_time)
    if args.deepspeed:
        print(f"TdH: deepspeed is on")
        increment = (
        ...
        print(f"TdH: deepspeed is off")
        update_successful, grad_norm, num_zeros_in_grad = optimizer.step(args, timers)
    timers("optimizer").stop()

    print(f"TdH: gather step")
    # Gather params.
    ...
    print(f"TdH: updating learning rate")
...
class interop_tool_logger:
    ...
    def add_scalar(
        ...
        if self.wandb_writer and InteropLoggingTool.WANDB in tool_list:
        ...
        if args.log_optimizer_states_to_tensorboard and optimizer is not None:
            ...
            if not hasattr(optimizer, "state"):
                assert hasattr(optimizer, "optimizer"), (
                    f"Optimizer must have optimizer property."
                )
...
def train(
    ...
    print(f"TdH: importing torch stuff")
    ...
    with profile(
        ...
        while iteration < args.train_iters and (
            ...
            print(f"TdH in while loop at iteration {iteration}")
            ...
            print(f"TdH: calling train_step at iteration {iteration}")
...
def evaluate_and_print_results(
    ...
    if (
        process_non_loss_data_func is not None
        and writer.is_enabled()
        and is_last_rank()
    ):
