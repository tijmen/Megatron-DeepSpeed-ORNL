...
try:
    ...
    print(f"TdH: flash_attn_varlen_func import error: {e}")
...
class ParallelMLP(MegatronModule):
    ...
    def __init__(self, config, moe=False, enable_expert_tensor_parallelism=False):
        ...
        #if config.gated_linear_unit: # use ffn_hidden_size for both linear layers (gate and up)
        ffn_hidden_size *= 2 # FORCE SWIGLU

        # Project to 4h. If using swiglu double the output width, see https://arxiv.org/pdf/2002.05202.pdf
        #print(f"TdH: defining dense_h_to_4h. Inputs:") 
        #print(f"TdH: ffn_hidden_size: {ffn_hidden_size}")
        #print(f"TdH: config is {config}")
        ...
        if self.ds_sequence_parallel_fpdt:
            self.fpdt_FFN_chunk_size = int(
                args.ds_sequence_parallel_fpdt_chunk_size
    ...
    def forward(self, hidden_states):
        if self.ds_sequence_parallel_fpdt:
            output, output_bias = FPDT_FFN.apply(
                hidden_states,
...
class ParallelAttention(MegatronModule):
    ...
    def __init__(
        ...
        if self.use_flash_attn:
            ...
            if args.use_flash_attn_v1:
                assert flash_attn_unpadded_func != None, (
                    "Cannot import FlashAttention v1 "
                )
            if args.use_flash_attn_v2:
                assert flash_attn_varlen_func != None, (
            ...
            if args.use_flash_attn_builder:
                assert flash_attn_builder != None, (
        ...
        if self.use_flash_attn_triton:
            ...
            print(f"TdH: using flash attention in ParallelAttention")
            local_attn = FlashSelfAttention(
                causal=True, attention_dropout=config.attention_dropout
            )
        else:
            #print(f"TdH: using CoreAttention in ParallelAttention, NOT flash")
    ...
    def forward(
        ...
        print(f"TdHTdH: forward method starting. rotary_pos_emb is None? {rotary_pos_emb is None}")
        print(f"TdHTdH: self.enable_ds_sequence_parallel: {self.enable_ds_sequence_parallel}")
...
def launch_chunk_attention(args, config):
    assert (
        args.num_attention_heads % parallel_state.get_sequence_parallel_world_size()
        == 0
    )
    ...
    if config.perform_initialization:
        tensor_parallel.layers._initialize_affine_weight_gpu(
            qkv_linear_weight, config.init_method, partition_dim=0
        )
    qkv_linear_bias = None
    if args.add_bias_linear:
        qkv_linear_bias = Parameter(
            torch.empty(
                projection_size + 2 * kv_projection_size,
                device=get_accelerator().current_device_name(),
                dtype=config.params_dtype,
    ...
    if config.perform_initialization:
        tensor_parallel.layers._initialize_affine_weight_gpu(
            qkv_dense_weight, config.output_layer_init_method, partition_dim=0
        )
    qkv_dense_bias = None
    if args.add_bias_linear:
        qkv_dense_bias = Parameter(
            torch.empty(
                config.hidden_size,
                device=get_accelerator().current_device_name(),
                dtype=config.params_dtype,
...
class ParallelTransformerLayer(MegatronModule):
    ...
    def __init__(
        ...
        # Self attention.
        self.ds_sequence_parallel_fpdt = args.ds_sequence_parallel_fpdt
        self.ds_sequence_parallel_fpdt_offloading = (
            args.ds_sequence_parallel_fpdt_offloading
        )
    ...
    def forward(
        ...
        # Self attention. Assume no cpu offloading
...
class ParallelTransformerLayerPipe(ParallelTransformerLayer):
    ...
    def forward(self, inputs, **kwargs):
        ...
        if torch.is_tensor(inputs) or len(inputs) == 1:
            assert not self.input_aggregated_moe_loss, (
